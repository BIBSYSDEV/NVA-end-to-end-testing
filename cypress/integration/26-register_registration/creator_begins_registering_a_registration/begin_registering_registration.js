...
And('they enter {string}', () => {
  cy.get('[data-testid=new-registration-link-field] > div > input').type(dcIdentifierLink);
});

//   @441
//   Scenario: Creator begins registering with a Link with data from DC and DCTERMS meta tags
And('they enter {string}', () => {
  cy.get('[data-testid=new-registration-link-field] > div > input').type(dcTermLink);
});

//   @442
//   Scenario: Creator begins registering with a Link with data from Open Graph tag
And('they enter {string}', () => {
  cy.get('[data-testid=new-registration-link-field] > div > input').type(openGraphLink);
});

//   @2208
//   Scenario: Creator begins registering with a Link with data from Highwire tag
When('they enter {string}', () => {
  cy.get('[data-testid=new-registration-link-field] > div > input').type(highwireLink);
});

//   @2370
//   Scenario: Creator begins registering with a Link with data from schema.org
When('they enter {string}', () => {
  cy.get('[data-testid=new-registration-link-field] > div > input').type(schemaLink);
});
