...
import { contributors, contributorsCommon, fillInField, registrationFields, resourceTypes } from './save_registration';
import { DataTable } from '@badeball/cypress-cucumber-preprocessor';
import { Field, FieldImpl, FieldMap, isField } from './fields';
...
Cypress.Commands.add('checkMenu', (table: string[]) => {
  cy.get(`[data-testid=${dataTestId.header.generalMenuButton}]`).click();
  table.forEach((row) => {
...
Cypress.Commands.add('loginCognito', (userId) => {
  return new Cypress.Promise((resolve, reject) => {
    ...
    identityServiceProvider.adminSetUserPassword(passwordParams, (err: any, data: any) => {
      if (data) {
        identityServiceProvider.adminInitiateAuth(authorizeUser, async (err: any, data: any) => {
          if (data) {
...
Cypress.Commands.add('testDataTestidList', (dataTable: DataTable, values) => {
...
Cypress.Commands.add('mockUpdatePerson', (userId) => {
  ...
  cy.intercept('DELETE', `https://api.${stage}.nva.aws.unit.no/person/1234567890/identifiers/feideid/delete`, (req) => {
    ...
    const author: authorMap = { ...mockPerson(userId), feideids: [userId] };
...
Cypress.Commands.add('checkResourceFields', (type, subtype) => { });
...
Cypress.Commands.add('fillInCommonFields', (type, subtype) => {
  Object.keys(registrationFields).forEach((key) => {
    ...
    Object.keys(registrationFields[key]).forEach((subkey) => {
      const field: string | Field | undefined = registrationFields[key][subkey];
      if (field instanceof FieldImpl) { fillInField(field); }
    });
  });
});
...
Cypress.Commands.add('fillInResourceType', (type, subtype) => {
  ...
  cy.get(`[data-testid=publication-context-type-${type.replace(' ', '-')}]`).click({ force: true });
  cy.get(`[data-testid=publication-instance-type]`).click();
  cy.get(`[data-testid=publication-instance-type-${subtype.replace(' ', '-')}]`).click({ force: true });
  Object.keys(resourceTypes[type][subtype]).forEach((key) => {
    if (key !== 'contributorType') {
      const field = resourceTypes[type][subtype][key];
      if (isField(field)) { fillInField(field); }
    }
  });
  if ('contributorType' in resourceTypes[type]) {
    ...
    const field = resourceTypes[type]['contributorType'];
    if (field instanceof FieldImpl) fillInField(field);
  }
});
