...
Given('fill in values for all fields', () => {
  cy.get('@resourceType').then((resourceType) => {
    ...
    cy.fillInContributors(artisticContributorRoles[resourceType.toString()]);
  });
...
Then('they can see the values in the Registration Wizard', () => {
  ...
  cy.get('@resourceType').then((subtype) => {
    ...
    fields[subtype.toString()].forEach((field: string) => {
      cy.checkField(field);
    });
    cy.checkContributors(artisticContributorRoles[subtype.toString()]);
  });
});
