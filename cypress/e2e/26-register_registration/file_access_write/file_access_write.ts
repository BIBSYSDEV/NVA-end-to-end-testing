...
When('the user attempts to "write-metadata"', () => {
...
Then('the action outcome is {string}', (outcome: string) => {
  cy.get('@fileType').then((fileType) => {
    if (outcome === 'Allowed') {
      cy.get('@userRole').then((userRole) => {
        if (userRole.toString() !== 'Everyone else') {
          ...
          if (fileType.toString() === OPEN_FILE || fileType.toString() === PENDING_OPEN_FILE) {
