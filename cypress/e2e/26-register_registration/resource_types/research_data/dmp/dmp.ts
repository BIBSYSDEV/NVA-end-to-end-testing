...
import { CategoryTypes, userUnitResearchDataDmp } from '../../../../../support/constants';
...
When('the User searches for a Publisher in the Publisher field', () => {
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.publisherField).type('Norges forskningsråd');
...
Then('the User replaces the prefilled Publisher with a Publisher from the search result', () => {
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.publisherField).within(() => {
...
Then('the user sees the saved link', () => {
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.externalLink).within(() => {
    cy.contains('https://sikt.no/');
  });
});
Then('the user has the option to remove the saved link', () => {
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.removeRelationButton('')).should('be.visible');
});
...
When('the user removes an external link', () => {
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.removeRelationButton('')).click();
  cy.getDataTestId(dataTestId.confirmDialog.acceptButton).click();
});
Then('the user sees the external link is removed', () => {
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.externalLink).should('not.exist');
  cy.getDataTestId(dataTestId.registrationWizard.resourceType.removeRelationButton('')).should('not.exist');
});
