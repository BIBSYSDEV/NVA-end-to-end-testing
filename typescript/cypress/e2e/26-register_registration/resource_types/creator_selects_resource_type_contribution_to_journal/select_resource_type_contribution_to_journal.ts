...
Then('they see fields:', (dataTable: DataTable) => {
  const fields = Object.values({ ...journalFields });
...
Then('they select the Resource subtype "Corrigendum"', () => {
  cy.get(`[data-testid=${journalSubtypes['Corrigendum']}]`).click({
    force: true,
  });
...
Given('they enter numbers for "Pages from" and "Pages to"', () => {
...
Given('the number for "Pages from" is greater than the number for "Pages to"', () => {
...
When('they see a disabled field for Journal based on selected Journal article', () => {
