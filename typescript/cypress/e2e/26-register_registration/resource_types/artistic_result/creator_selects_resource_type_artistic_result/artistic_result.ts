...
Given('they select any Subtype', () => {
  cy.get('[data-testid^=resource-type-chip-]').first().next().click();
});
...
Then('they see a new field where they can enter actual Type', () => {
  cy.get(`[data-testid=${dataTestId.registrationWizard.resourceType.artisticSubtypeDescription}]`).should('be.visible');
});
