...
When('they click {string} under {string}', (button: string, section: string) => {
...
Then('they see that the list has the fields "Username" and "Name" for each user', () => {
  cy.get('@section').then((section) => {
    cy.get(`[data-testid=${userAdministrationSections[section.toString()]}]`).within(() => {
...
Then('they see a button "Remove" that is enabled for each user', () => {
  cy.get('@role').then((role) => {
    cy.get(`[data-testid^=${userAdministrationRemoveRoleButtons[role.toString()]}]`).should('have.length', 5);
  });
});
...
Then('they see a list of employees with an "Add" button', () => {
  cy.get('@roleModal').within(() => {
    cy.get('@button').then((button) => {
      cy.get(`[data-testid^=${userAdministrationAddRoleButtons[button.toString()]}]`).should('have.length.at.least', 1);
    });
  });
});
